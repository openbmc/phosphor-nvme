...
void Nvme::createNVMeInventory_present()
{
    ...
    for (const auto config : configs)
    {
        presentPath = "/system/chassis/motherboard/nvme" + config.index + "_prsnt";
...
void Nvme::readNvmeData(NVMeConfig& config)
{
    ...
    std::string presentPath = "/xyz/openbmc_project/inventory/system/chassis/motherboard/nvme" + config.index + "_prsnt";
    NVMeData nvmeData;
    ...
    if (iter == nvmes.end())
    {
        ...
        setNvmeInventoryProperties_present(true, nvmeData, presentPath);
...
void Nvme::read()
{
    ...
    for (auto config : configs)
    {
        ...
        presentPath = "/xyz/openbmc_project/inventory/system/chassis/motherboard/nvme" + config.index + "_prsnt";
        ...
        if (config.presentPin)
        {
            ...
            if (getGPIOValueOfNvme(devPresentPath) != IS_PRESENT)
            {
                ...
                nvmeData = NVMeData();
                setNvmeInventoryProperties(nvmeData, inventoryPath);
                setNvmeInventoryProperties_present(false, nvmeData, presentPath);
